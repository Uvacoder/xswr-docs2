...
`make`, `Schema.make`, and Object constructors all accept `initialize` as last optional parameter (default true) in order to initialize the object with the current state.

It can be set to false for optimizing performances when you just need to call `mutate` and don't need the data, for example:
