...
function getAllDataSchema() {
  async function normalizer(data: Data[], more: XSWR.NormalizerMore) {
    return await Promise.all(data.map(it => getDataNormal(it, more)))
  }
...
function useData(id: string) {
  return XSWR.use(getDataSchema, [id])
}
...
export function getVideoSchema(id: string) {
  async function normalizer(video: VideoData, more: XSWR.NormalizerMore) {
    const author = await getProfileRef(video.author, more) // Object
    const comments = await Promise.all(video.comments.map(it => getCommentNormal(it, more))) // Array
    return { ...video, author, comments }
  }

  return XSWR.single<VideoData | NormalizedVideoData, Error>(
