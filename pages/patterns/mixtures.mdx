...
```tsx
export function useAutoFetchMixture(schema: XSWR.Schema) {
  const handle = XSWR.use(schema)
...
export function useFetchAndDebugMixture(schema: XSWR.Schema, label: string) {
  const handle = XSWR.use(schema)
...
export function useCatAutoFetchMix(id: string) {
  return useAutoFetchMixture(getCatSchema(id))
}

export function useCatFetchAndDebugMix(id: string) {
  return useFetchAndDebugMixture(getCatSchema(id), `cat#${id}`)
}
```
...
export function useDogAutoFetchMix(id: string) {
  return useAutoFetchMixture(getDogSchema(id))
}

export function useDogFetchAndDebugMix(id: string) {
  return useFetchAndDebugMixture(getDogSchema(id), `dog#${id}`)
}
```