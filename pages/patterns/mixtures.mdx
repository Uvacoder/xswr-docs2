...
```tsx
export function useAutoFetchMix(schema: XSWR.Schema) {
...
export function useFetchAndDebugMix(schema: XSWR.Schema, label: string) {
  const handle = XSWR.use(schema)
...
export function useCatAutoFetchMix(id: string) {
  return useAutoFetchMix(getCatSchema(id))
}

export function useCatFetchAndDebugMix(id: string) {
  return useFetchAndDebugMix(getCatSchema(id), `cat#${id}`)
}
```
